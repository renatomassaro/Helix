defmodule Helix.Software.Request.File.Upload do
  ...
  def check_params(request, session) do
    ...
    unsafe_storage_id =
      if Map.has_key?(request.unsafe, "storage_id") do
        ...
        StorageQuery.get_main_storage(session.context.endpoint.server_id)
      end
    ...
    do
      ...
      :bad_access ->
        bad_request(request, :upload_self)
  ...
  def handle_request(request, session) do
    ...
    upload =
      FilePublic.upload(gateway, endpoint, tunnel, storage, file, relay)
