defmodule Helix.Software.Request.File.DownloadTest do

  use Helix.Test.Case.Integration

  alias Helix.Process.Query.Process, as: ProcessQuery
  alias Helix.Software.Query.Storage, as: StorageQuery
  alias Helix.Software.Request.File.Download, as: FileDownloadRequest
  ...
  describe "check_params/2" do
    test "validates and casts expected data" do
      ...
      assert {:ok, request} =
        FileDownloadRequest.check_params(request, @session)
    ...
    test "uses main storage_id if none was given" do
      ...
      gateway_storage_id = StorageQuery.get_main_storage_id(gateway.server_id)
      file_id = SoftwareHelper.id()
      ...
      assert request.params.file_id == file_id
      assert request.params.storage_id == gateway_storage_id
    end
  ...
  describe "check_permissions/2" do
    test "accepts when everything is OK" do
      ...
      gateway_storage = StorageQuery.get_main_storage(gateway)
      ...
      assert {:ok, request} =
        RequestHelper.check_permissions(FileDownloadRequest, request, session)


      assert request.meta.gateway == gateway
      assert request.meta.endpoint == endpoint
      assert request.meta.file == file
      assert request.meta.storage == gateway_storage
    end
    ...
    test "rejects if invalid file was passed" do
      ...
      # Now we'll create the file... on the gateway! So, the gateway is
      # attempting to download a file on the endpoint. But this file
      # exists on the gateway itself. OMFG!
    ...
    test "rejects if invalid storage was passed" do
      ...
      # Valid file that we'll be downloading
      file = SoftwareSetup.file!(server_id: endpoint.server_id)
      ...
      # Now we'll try again... With the endpoint storage. So, the gateway
      # is attempting to download a file that exists on endpoint. But the
      # download will target the endpoint's own storage id! OMFG!
      endpoint_storage_id = StorageQuery.get_main_storage_id(endpoint)


      # Notice `storage_id` is random
      params = %{
        "file_id" => to_string(file.file_id),
        "storage_id" => to_string(endpoint_storage_id)
      }
  ...
  describe "handle_request/2" do
    test "starts the process" do
      ...
      assert {:ok, _request} =
        RequestHelper.handle_request(FileDownloadRequest, request, session)
